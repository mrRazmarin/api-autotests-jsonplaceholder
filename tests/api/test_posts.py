...
@allure.epic("API Тестирование")
@allure.feature("Ресурс Posts")
@allure.story("Получение ранее созданного поста по id")
@allure.title("Получение созданного поста")
@allure.tag("smoke", "api", "regress")
@pytest.mark.api
def test_get_post_by_id_chain(posts_client):
    faker = Faker()
    ...
    with allure.step("Отправка POST запроса на /posts"):
        ...
        created_post_dto = SinglePostResponseDto(**response.json())
        new_post_id = created_post_dto.id

    with allure.step(f"Отправка GET запроса на /posts/{new_post_id}"):
        response_get_post = posts_client.get_post_by_id(new_post_id)
        assert response_get_post.status_code == 200

    with allure.step("Сравнение отправленных и полученных данных"):
        get_post_dto = SinglePostResponseDto(**response_get_post.json())

        assert get_post_dto.id == new_post_id

        assert get_post_dto.title == title, f"Заголовок не совпадает! Ожидали: {title}, Получили: {get_post_dto.title}"
        assert get_post_dto.body == body, "Тело поста не совпадает!"
        assert get_post_dto.user_id == user_id, "User ID не совпадает!"

    with allure.step("Прикрепление отчета"):
        pretty_json = json.dumps(response_get_post.json(), indent=4, ensure_ascii=False)
        allure.attach(
            pretty_json,
            name="Полученный пост",
            attachment_type=allure.attachment_type.JSON
